...
#define WinRTKey(name) (unsigned char)Windows::System::VirtualKey::name
    ...
    void KbmInitialize()
    {
        ...
        if (FAILED(s_Keyboard->SetCooperativeLevel( l_hWnd, DISCL_FOREGROUND | DISCL_NONEXCLUSIVE)))
        ...
        if (FAILED(s_Mouse->SetCooperativeLevel( l_hWnd, DISCL_FOREGROUND | DISCL_EXCLUSIVE)))
    ...
    void KbmUpdate()
    {
        ...
        if (foreground !=  l_hWnd // wouldn't be able to acquire
            || !visible)
        {
            KbmZeroInputs();
        }
...
void GameInput::Initialize(HWND p_hWnd)
{
    // For Windows 8
    //	XInputEnable(TRUE);
