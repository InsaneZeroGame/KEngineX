...
namespace Renderer {
    ...
    void DX12ColorBuffer::CreateDerivedViews(ID3D12Device* Device, DXGI_FORMAT Format, uint32_t ArraySize, uint32_t NumMips)
    {
        ...
        if (m_SRVHandle.cpu_handle.ptr == 0)
        {
            m_RTVHandle = DX12GpuDevice::GetGpuDevice().GetDescriptorHandle(D3D12_DESCRIPTOR_HEAP_TYPE_RTV);
            m_SRVHandle = DX12GpuDevice::GetGpuDevice().GetDescriptorHandle(D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV);
        }

        ID3D12Resource* Resource = m_pResource.Get();

        // Create the render target view
        Device->CreateRenderTargetView(Resource, &RTVDesc, m_RTVHandle.cpu_handle);

        // Create the shader resource view
        Device->CreateShaderResourceView(Resource, &SRVDesc, m_SRVHandle.cpu_handle);
        ...
        {
            if (m_UAVHandle[i].cpu_handle.ptr == 0)
                m_UAVHandle[i] = DX12GpuDevice::GetGpuDevice().GetDescriptorHandle(D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV);

            Device->CreateUnorderedAccessView(Resource, nullptr, &UAVDesc, m_UAVHandle[i].cpu_handle);

            UAVDesc.Texture2D.MipSlice++;
        }
    }

    void DX12ColorBuffer::CreateFromSwapChain(const std::wstring& Name, ID3D12Resource* BaseResource)
    {
        ...
        //DX12GpuDevice::GetGpuDevice().GetDX12Device()->CreateUnorderedAccessView(m_pResource.Get(), nullptr, nullptr, m_UAVHandle[0]);

        m_RTVHandle = DX12GpuDevice::GetGpuDevice().GetDescriptorHandle(D3D12_DESCRIPTOR_HEAP_TYPE_RTV);
        DX12GpuDevice::GetGpuDevice().GetDX12Device()->CreateRenderTargetView(m_pResource.Get(), nullptr, m_RTVHandle.cpu_handle);
    }

    void DX12ColorBuffer::Create(const std::wstring& Name, uint32_t Width, uint32_t Height, uint32_t NumMips,
        DXGI_FORMAT Format, D3D12_GPU_VIRTUAL_ADDRESS VidMem)
    {
        ...
        CreateDerivedViews(DX12GpuDevice::GetGpuDevice().GetDX12Device(), Format, 1, NumMips);
    }
    ...
    void DX12ColorBuffer::CreateArray(const std::wstring& Name, uint32_t Width, uint32_t Height, uint32_t ArrayCount,
        DXGI_FORMAT Format, D3D12_GPU_VIRTUAL_ADDRESS VidMem)
    {
        D3D12_RESOURCE_FLAGS Flags = CombineResourceFlags();
        ...
        CreateTextureResource( Name, ResourceDesc, ClearValue, VidMem);
        CreateDerivedViews(DX12GpuDevice::GetGpuDevice().GetDX12Device(), Format, ArrayCount, 1);
    }
    ...
    //void DX12ColorBuffer::GenerateMipMaps(CommandContext& BaseContext)
    //{
    //    if (m_NumMipMaps == 0)
    //        return;
    //
