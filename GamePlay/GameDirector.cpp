...
void gameplay::GameDirector::Init()
{
    ...
    auto res = std::async(std::launch::async,&assetlib::AssetManager::LoadScene,m_asset_manager, "kengine.fbx");
    //m_asset_manager->LoadScene("crytek-sponza.obj");
}

void gameplay::GameDirector::Update()
{
    m_renderer->SetCurrentScene(m_asset_manager->GetScene("kengine.fbx"));
}
